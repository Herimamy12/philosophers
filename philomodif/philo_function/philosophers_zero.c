...
t_philo	*new_philo(t_data *data)
{
	...
	while (i < lim)
	{
		...
		philo[i].l_state = data->fork_state[i];
		if (i < lim - 1)
		{
			philo[i].r_fork.fork = data->fork[i + 1].fork;
			philo[i].r_state = data->fork_state[i + 1];
		}
		else
		{
			philo[i].r_fork.fork = data->fork[0].fork;
			philo[i].r_state = data->fork_state[0];
		}
		i++;
	}
	return (philo);
...
t_data	*new_data(int ac, char **av)
{
	...
	fork = new_fork (param);
	data->fork = fork;
	fork_state = new_fork_state (param);
	data->fork_state = fork_state;
